...
#        handle -lmariadb   -pthread -lz -ldl -lm -lpthread -lssl -lcrypto  -Wl,--no-undefined   and other strings inserted that might contain
...
my $pkgincludedir = fix_path('/usr/local/include/mysql', "include/mysql", "include");
...
$flags->{libs} =
  [@ldflags,@lib_opts,'','','',''];
$flags->{libs_r} =
  [@ldflags,@lib_r_opts,'','-lmariadb   -pthread -lz -ldl -lm -lpthread -lssl -lcrypto  -Wl,--no-undefined  ',''];
$flags->{embedded_libs} =
  [@ldflags,@lib_e_opts,'dl','','-lmariadb   -pthread -lz -ldl -lm -lpthread -lssl -lcrypto  -Wl,--no-undefined  ','',''];
