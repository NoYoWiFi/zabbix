...
case ${1} in
    ...
    "proxy")
        echo "proxy"
        chmod 766 /usr/share/doc/zabbix-proxy-mysql-*/schema.sql.gz
        zcat /usr/share/doc/zabbix-proxy-mysql-*/schema.sql.gz | mariadb -h 127.0.0.1 -uzabbix -p$DPassword zabbix;
        ;;
    *)
        echo "Nothing to do"
        ;;
...
case ${1} in
    ...
    *)
        #![配置nginx]
        \cp ./patch/nginx.conf /etc/opt/rh/rh-nginx116/nginx/nginx.conf
        \cp ./patch/zabbix.conf /etc/opt/rh/rh-nginx116/nginx/conf.d
        ...
        sed -i -e "/listen/d" /etc/opt/rh/rh-nginx116/nginx/conf.d/zabbix.conf
        sed -i -e "/server {/a\ \tlisten 8080;\n\tlisten 8443 ssl;" /etc/opt/rh/rh-nginx116/nginx/conf.d/zabbix.conf
        sed -i -e "/ssl_/d" /etc/opt/rh/rh-nginx116/nginx/conf.d/zabbix.conf
        sed -i -e "/if /,+2d" /etc/opt/rh/rh-nginx116/nginx/conf.d/zabbix.conf
        ...
        sed -i -e "/^\max_execution_time/s/=.*/= 600/" /etc/opt/rh/rh-php73/php.ini
        sed -i -e "/^\max_input_time/s/=.*/= 600/" /etc/opt/rh/rh-php73/php.ini
        sed -i -e "/^\;date.timezone/s/=.*/= Asia\/Shanghai/" /etc/opt/rh/rh-php73/php.ini
        sed -i -e "/^\;date.timezone/s/^;//" /etc/opt/rh/rh-php73/php.ini
        ;;
esac
#![配置snmptrap]
...
case ${1} in
    ...
    *)
        ...
        \cp patch/frontend_5.0.mo /usr/share/zabbix/locale/zh_CN/LC_MESSAGES/frontend.mo
