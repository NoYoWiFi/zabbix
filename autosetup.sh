...
if [ $? -ne 0 ];then
    ...
    dnf makecache -y
    rpm -ivhU --force ./packages/pgdg-redhat-repo-42.0-46PGDG.noarch.rpm
#    https://mirrors.aliyun.com/postgresql/repos/yum/common/redhat/rhel-9-x86_64/pgdg-redhat-repo-42.0-46PGDG.noarch.rpm
#    sed -i -e "/rhel-\$releasever-\$basearch/s/rhel-\$releasever-\$basearch/rhel-9-x86_64/" /etc/yum.repos.d/pgdg-redhat-all.repo
    escape_spec_char() {
    local var_value=$1
