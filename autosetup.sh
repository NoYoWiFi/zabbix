...
case ${1} in
    "proxy")
        echo "proxy"
        /usr/bin/rpm -ivhU --force --nodeps packages/zabbix-agent2* packages/zabbix-get* packages/zabbix-java-gateway* \
        packages/zabbix-proxy-pgsql* packages/zabbix-selinux-policy* \
        packages/zabbix-sender* packages/zabbix-sql-scripts*
...
echo ${DPassword} | passwd --stdin postgres
# 归其为mysql用户
...
/usr/local/bin/postgresql-setup initdb
...
sed -i -e "/^max_connections/s/=.*/= 2000/" /var/lib/pgsql/data/postgresql.conf
\cp ./pgsql/pg_hba.conf /var/lib/pgsql/data/
systemctl start postgresql
sleep 3
#![创建zabbix数据库导入create_server_${GV_ARR_ENV[GV_ZABBIX_POSTFIX]}汉化模板]
echo "create user zabbix with password '${DPassword}';" | sudo -u postgres bash -c '. /var/lib/pgsql/.bashrc; /usr/local/bin/psql;'
echo "alter user postgres with password '${DPassword}';" | sudo -u postgres bash -c '. /var/lib/pgsql/.bashrc; /usr/local/bin/psql;'
echo "create database zabbix;" | sudo -u postgres bash -c '. /var/lib/pgsql/.bashrc; /usr/local/bin/psql;'
echo "alter database zabbix owner to zabbix;" | sudo -u postgres bash -c '. /var/lib/pgsql/.bashrc; /usr/local/bin/psql -dzabbix;'
echo "grant all on database zabbix to zabbix;" | sudo -u postgres bash -c '. /var/lib/pgsql/.bashrc; /usr/local/bin/psql -dzabbix;'
...
case ${1} in
    "trans")
        echo "trans"
        gunzip < /usr/share/zabbix-sql-scripts/postgresql/server.sql.gz | sudo -u postgres bash -c '. /var/lib/pgsql/.bashrc; /usr/local/bin/psql'
        systemctl restart postgresql-${GV_ARR_ENV[GV_PGSQL_VERSION]}
        ;;
    "install")
        echo "install"
        zcat /usr/share/zabbix-sql-scripts/postgresql/create_server_${GV_ARR_ENV[GV_ZABBIX_POSTFIX]}_pgsql.sql.gz | sudo -u postgres bash -c 'export PGPASSWORD='${DPassword}'; . /var/lib/pgsql/.bashrc; /usr/local/bin/psql -Uzabbix -hlocalhost;'
        #![开启postgresql timescaleDB插件]
        #sed -i "/#dynamic_library_path/s/#dynamic_library_path/dynamic_library_path/" /var/lib/pgsql/data/postgresql.conf
        #sed -i "/dynamic_library_path/s/dynamic_library_path =.*/dynamic_library_path = \'\/usr\/local\/lib\/postgresql:\/usr\/local\/src\/perl5-5\.26\.3:\/usr\/local\/lib:\/usr\/local\/lib64\'/" /var/lib/pgsql/data/postgresql.conf
        #dynamic_library_path = '$libdir'
        echo "CREATE EXTENSION IF NOT EXISTS timescaledb CASCADE;" | sudo -u postgres bash -c 'export PGPASSWORD='${DPassword}'; . /var/lib/pgsql/.bashrc; /usr/local/bin/psql -Uzabbix -hlocalhost;'
        #![为timescaleDB分区]
        echo "\c zabbix;\i /usr/share/zabbix-sql-scripts/postgresql/timescaledb.sql;" | sudo -u postgres bash -c 'export PGPASSWORD='${DPassword}'; . /var/lib/pgsql/.bashrc; /usr/local/bin/psql -Uzabbix -hlocalhost;'
        ;;
    "proxy")
        echo "proxy"
        chmod 766 /usr/share/zabbix-sql-scripts/postgresql/proxy.sql
        echo "\c zabbix;\i /usr/share/zabbix-sql-scripts/postgresql/proxy.sql;" | sudo -u postgres bash -c 'export PGPASSWORD='${DPassword}'; . /var/lib/pgsql/.bashrc; /usr/local/bin/psql -Uzabbix -hlocalhost;'
        ;;
    *)
        echo "Nothing to do"
        ;;
...
case ${1} in
    ...
    *)
        ...
        mkdir /var/log/loki
        touch /var/log/loki/zabbix_php-fpm_slow.log
        ...
        #![配置php-fpm]
        sed -i -e "/^\listen =/s/= .*/= \/var\/run\/php-fpm\/zabbix\.sock/" /etc/php-fpm.d/zabbix.conf
        sed -i "/date\.timezon/s/\; php_value\[date\.timezone\] = Europe\/Riga/php_value[date.timezone] = Asia\/Shanghai/" /etc/php-fpm.d/zabbix.conf
        #cp /usr/share/doc/zabbix-agent-*/userparameter_mysql.conf /etc/zabbix/zabbix_agentd.d/
        sed -i -e "/^\post_max_size/s/=.*/=32M/" /etc/php.ini
        sed -i -e "/^\max_execution_time/s/=.*/= 600/" /etc/php.ini
        sed -i -e "/^\max_input_time/s/=.*/= 600/" /etc/php.ini
...
/usr/local/bin/net-snmp-create-v3-user -ro -A Admin@zabbix -a MD5 -X Admin@zabbix -x DES zabbix
case ${1} in
    "proxy")
        echo "proxy"
        ;;
