...
from ssh.sshExportFirewall import MyThreadExportFirewall


def xls_cfg_hostName():
    workbook = xlrd.open_workbook(xls.var_file.xls_04)
    for n_1 in xrange(workbook.sheets()[0].nrows):
        if n_1 == 0:
            ...
            try:
                m = MyThreadExportFirewall(cur_num, total_num, ssh_ip, port, pwd)
                m.start()
                m.join()
