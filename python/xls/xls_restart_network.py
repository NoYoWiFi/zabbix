...
from ssh.sshRestartNetwork import MyThreadRestartNetwork


def xls_cfg_hostName():
    workbook = xlrd.open_workbook(xls.var_file.xls_03)
    for n_1 in xrange(workbook.sheets()[0].nrows):
        if n_1 == 0:
            ...
            try:
                m = MyThreadRestartNetwork(cur_num, total_num, ssh_ip, port, pwd)
                m.start()
                m.join()
