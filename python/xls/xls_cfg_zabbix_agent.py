...
import xls.var_file
from ssh.sshCfgZabbixAgent import MyThreadCfgZabbixAgent


def xls_cfg_zabbix_agent():
    ...
    for n_1 in xrange(workbook.sheets()[0].nrows):
        if n_1 == 0:
            ...
            try:
                zabbix_server_ip = sys.argv[1]
                m = MyThreadCfgZabbixAgent(cur_num, total_num, ssh_ip, port, pwd, zabbix_server_ip)
                m.start()
                m.join()
