...
detect_os ()
{
  ...
  if [[ "$os" = "ol" || "$os" = "el" || "$os" = "rocky" ||  "$os" = "almalinux" ||  "$os" = "centos" ]] && [ $(($dist)) \> 7 ]; then
    _skip_pygpgme=1
...
finalize_yum_repo ()
{
  if [ "$_skip_pygpgme" = 0 ]; then
    echo "Attempting to install pygpgme if required (only for older OSes) to verify GPG signatures..."
...
main ()
{
  repo_config_name=timescale_timescaledb
  detect_os
  curl_check


  yum_repo_config_url="https://packagecloud.io/install/repositories/timescale/timescaledb/config_file.repo?os=${os}&dist=${dist}&source=script"
